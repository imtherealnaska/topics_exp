...
2. *Broadcast rules* :
                       <br/>a.  when operating on 2 arrays numpy compares the shape element-wise , starts with the trailing dimensions 
                           and works its way forward</br>
                       b.  if they are equal</br> 
                       c.  if one of them is 1 ,</br> 
                       d.  image(256*256*3) scale (3) result  , dimension with size 1 is stretched or copied</br> 
                       e.  A(4d array) 8*1*6*1 and B(7*1*5) the result is 8*7*6*5
